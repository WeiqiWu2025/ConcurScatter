...
global reference_sequence_length;
reference_sequence_length = 32; % in bits
...
for i = 1:S
    ...
    while  n<=maxNumPackets
        ...
        % Initial tags data
        tagData = reshape(survey_randd(numTags*(numTagData-reference_sequence_length),tag_modulation),(numTagData-reference_sequence_length),[]).';
        reference_sequence = ones(numTags,reference_sequence_length);
        mod_reference_sequence = pskmod(reference_sequence,tag_modulation);
        tagData = [mod_reference_sequence,tagData];
        ...
        for idx_1 = 1:numTags
            bxSig{idx_1} = exSig(:,idx_1);
            bxSig{idx_1}(801:800+length(sig_mod)) = sig_mod(:,idx_1).*bxSig{idx_1}(801:800+length(sig_mod));
        end
...
%         rxFromTags = rx;
        ofdmDemod = survey_funcReceiver(rxFromTags(ind.HTData(1):ind.HTData(2)),cfgHT,1);
        ofdmDemod1 = ofdmDemod(:,2:1+numTagData);
        
        [~,original_ofdm_symbols] = survey_funcOFDMSymDerived(txPSDU,cfgHT);
        ...
        refData = mod_reference_sequence.';
        % phase correction
        for rx_idx7 = 1:numTags
            phOffset = angle(refData(:,rx_idx7).*conj(dec_tag(1:reference_sequence_length,rx_idx7)));
            phOffset = mean(phOffset);
            dec_tag(:,rx_idx7) = exp(1i*phOffset)*dec_tag(:,rx_idx7);
        end
