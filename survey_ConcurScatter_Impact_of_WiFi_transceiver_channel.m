...
cfgHT.PSDULength = 500; % PSDU length in bytes % 64 %   PSUDLength * 8/26
...
tgnChannel_TX_RX.TransmitReceiveDistance = 1; % Distance in meters for NLOS
...
tgnChannel_Tag_RX.TransmitReceiveDistance = 1; % Distance in meters for NLOS
...
for i = 1:S
    ...
    while  n<=maxNumPackets
        ...
        WiFi_transceiver_link_snr = survey_ConcurScatter_funcChannelQuality2SNR(WiFi_transceiver_link);
        ...
        [decodedPSDU,flag] = survey_ConcurScatter_funcWiFiRX(rx_From_WiFi_TX,cfgHT);
        if flag == 0
            packetLoss(i,1) = packetLoss(i,1) + 1;
            for tt_tt = 1:numTags
                numBitErrs(i,tt_tt) = numBitErrs(i,tt_tt) + numTagData;
            end
            n = n+1;
            continue;
        end
        ...
        [~,original_ofdm_symbols] = survey_ConcurScatter_funcOFDMSymDerived(decodedPSDU,cfgHT);
